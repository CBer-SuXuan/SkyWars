...
从 SunGameCore `v1.2.2` 开始，queue 世界规则必须由使用方显式传入。SkyWars 通过 `queueWorldRules()` 声明等待大厅世界的 GameRule。
...
```java
arenaManager.createArenaAsync(mapConfig.templateWorld(), gameId, gameWorldRules())
...
- queue 世界只负责等待。
- 正式游戏世界从地图模板重新创建。
- queue 世界规则由 `queueWorldRules()` 显式声明。
- game 世界规则由 `gameWorldRules()` 显式声明。
- 游戏结束后正式游戏世界会被销毁。
...
Bukkit.getPluginManager().registerEvents(
        new CommonLifecycleListener<>(gameManager, lifecycleCallbacks(), protectionPolicy, lifecyclePolicy()),
        this
);
```

它负责监听生命周期事件，但具体行为由 SkyWars 显式提供：

- `lifecycleCallbacks()`：处理业务动作，例如进服入队、退出、淘汰、重生位置。
- `lifecyclePolicy()`：声明通用事件行为，例如隐藏进服/退服消息、进服清理、饥饿控制、死亡清掉落、自动重生、重生后类旁观。

这样 SunGameCore 不会替 SkyWars 写死生命周期玩法，SkyWars 自己声明需要保留哪些默认行为。
...
private LifecyclePolicy<SkyWarsGame> lifecyclePolicy() {
    return new LifecyclePolicy<>() {
        @Override
        public boolean hideJoinMessage(Player player) { return true; }

        @Override
        public boolean hideQuitMessage(Player player) { return true; }

        @Override
        public boolean clearInventoryOnJoin(Player player) { return true; }

        @Override
        public boolean resetPlayerOnJoin(Player player) { return true; }

        @Override
        public boolean autoRespawn(Player player, SkyWarsGame game) { return true; }

        @Override
        public boolean prepareSpectatorLikeOnRespawn(Player player, SkyWarsGame game) { return true; }
    };
}
```
